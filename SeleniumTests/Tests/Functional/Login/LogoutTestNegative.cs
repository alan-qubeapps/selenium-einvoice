...
namespace SeleniumTests.Tests.L_Functional.Login
{
    ...
    public static class ExcelDataReaderLogoutNegative
    {
        public static IEnumerable<object[]> GetPreventAccessTestData(string filePath, string sheetName)
        {
    ...
    public class LogoutTestNegative
    {
        ...
        private static string ExcelPath = Path.Combine(AppConfig.TestDataFolder, "LogoutTestDataNegative.xlsx");

        public static IEnumerable<object[]> PreventAccessTestData =>
        ExcelDataReaderLogoutNegative.GetPreventAccessTestData(ExcelPath, "PreventAccessTestData");
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Logout Page - Negative"; // You can make this dynamic if needed
        ...
        public void SetUp()
        {
            ...
            _logMessages.Clear();
            _moduleName = "Logout Page - Negative";
        ...
        [TestCaseSource(nameof(PreventAccessTestData))]
        public void TestLogout_PreventDashboardAccess(string isValidLogin)
        {
            ...
            if (isValidLogin == "1")
            {
                ...
                try
                {
                    ...
                    Assert.IsTrue(driver.Url.Contains("/auth/login"), "Expected to be on login page after logout.");
                }
                catch (Exception ex)
                {
                    LogStep($"❌ Logout failed: {ex.Message}");
                    Assert.Fail("Logout process failed.");
                }

                // --- Negative Test: Try to access dashboard URL directly ---
                LogStep("Attempting to access dashboard URL directly after logout");
                driver.Navigate().GoToUrl(AppConfig.BaseUrl + "/dashboard");
                WaitForUIEffect();

                _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"PostLogoutAccess_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                var postLogoutScreenshot = ((ITakesScreenshot)driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, postLogoutScreenshot.AsByteArray);

                // Negative assertion: user should NOT access dashboard
                Assert.IsFalse(driver.Url.Contains("/dashboard"), "❌ User should not access dashboard after logout. Negative test failed.");
                LogStep("✅ Negative test success: User cannot access dashboard after logout.");
            }
            else 
            {
                Assert.IsFalse(false, "❌ User Unable To Login. Negative test failed.");
                LogStep("❌ User Unable To Login. Negative test failed.");
            }
        }
