...
namespace SeleniumTests.Tests.G_Log.Paging
{
    ...
    public class Log_Imported_Paging
    {
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log Imported Page - Paging"; // You can make this dynamic if needed
        ...
        public void SetUp()
        {
            ...
            _moduleName = "Log Imported Page - Paging";
        ...
        public void Log_Imported_TestPagingNextButtonAndVerify()
        {
            LogStep("Switching to Log Imported tab.");
            ...
            LogStep("Waiting for table to be visible.");
            ...
            _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Imported_Page_{DateTime.Now:yyyyMMdd_HHmmss}.png");
        ...
        public void Log_Imported_TestPagingPreviousButtonAndVerify()
        {
            try
            {
                LogStep("Switching to Log Imported tab.");
                ...
                LogStep("📄 Original table captured.");
                ...
                try
                {
                    ...
                    LogStep(tableAfterNext != originalHtml
                        ? "✅ Table changed after 'Next'."
                        : "ℹ️ Table did not change after 'Next' (possibly already on last page).");
                }
                catch (Exception e)
                {
                ...
                try
                {
                    ...
                    LogStep(tableAfterPrev == originalHtml
                        ? "✅ Table returned to 'Previous'."
                        : "ℹ️ Table did not return to original (may not have left page 1).");
                }
                catch (Exception e)
                {
        ...
        public void Log_Imported_TestPagingClickLastIconAndVerify()
        {
            try
            {
                LogStep("Switching to Log Imported tab.");
        ...
        public void Log_Imported_TestPagingFirstButtonAndVerify()
        {
            string tableXPath = "//table/tbody/tr[1]";

            try
            {
                LogStep("📁 Navigating to 'Imported' tab");
                var LogImportedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
                By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Imported')]")
                ));
                LogImportedTab.Click();
        ...
        [TestCase("100")]
        public void Log_Imported_TestItemsPerPageVerify(string pageSizeValue)
        {
            LogStep("📁 Navigating to 'Imported' tab");
            var LogImportedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Imported')]")
        ...
        [TestCase("3")]
        public void Log_Imported_TestClickPageButtonIfExists(string pageNumber)
        {
            LogStep("📁 Navigating to 'Imported' tab");
            var LogImportedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Imported')]")
            ...
            {
                LogStep($"✅ No page {pageNumber} exists — test passes logically");
                Assert.IsTrue(true);
                return;
            }
