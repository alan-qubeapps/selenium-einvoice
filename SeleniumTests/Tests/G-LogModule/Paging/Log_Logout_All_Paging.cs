...
namespace SeleniumTests.Tests.G_Log.Paging
{
    ...
    public class Log_Logout_All_Paging
    {
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log Logout All Page - Paging"; // You can make this dynamic if needed
        ...
        public void SetUp()
        {
            ...
            _moduleName = "Log Logout All Page - Paging";
        ...
        public void Log_Logout_TestPagingNextButtonAndVerify()
        {
            LogStep("📁 Switching to Logout tab");
            var LogLogoutTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Logout all')]")));
            LogLogoutTab.Click();
            ...
            LogStep("Attempting to click 'Next' pagination button");
            ...
            if (isDisabled)
            {
                _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Logout_Page_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                var screenshot1 = ((ITakesScreenshot)_driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, screenshot1.AsByteArray);
                LogStep("⚠️ 'Next' button is disabled. Only one page available. Skipping.");
                Assert.IsTrue(true);
                return;
            }
        ...
        public void Log_Logout_TestPagingPreviousButtonAndVerify()
        {
            try
            {
                LogStep("📁 Switching to Logout tab");
                ...
                LogStep("📄 Captured original table content");

                try
                {
                    LogStep("Clicking 'Next' button to go forward");
                    ...
                    if (tableAfterNext != originalHtml)
                        ...
                        LogStep("ℹ️ Table did not change (maybe last page)");
                }
                catch (Exception e)
                {
                ...
                try
                {
                    LogStep("Clicking 'Previous' button to go back");
                    ...
                    if (tableAfterPrev == originalHtml)
                        ...
                        LogStep("ℹ️ Table did not return to original (maybe never left page 1)");
                }
                catch (Exception e)
                {
        ...
        public void Log_Logout_TestPagingClickLastIconAndVerify()
        {
            try
            {
                LogStep("📁 Switching to Logout tab");
                ...
                LogStep("✅ Clicked last page button");
                ...
                {
                    LogStep("⚠️ Table did not update after clicking 'Last'. Possibly already on last page.");
                }

                if (tableChanged)
                {
                    LogStep("✅ Table changed successfully after clicking 'Last'");
                    _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Logout_Page_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                    var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                    File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                    Assert.IsTrue(true);
        ...
        public void Log_Logout_TestPagingFirstButtonAndVerify()
        {
            try
            {
                LogStep("Switching to Log Logout tab");
                var LogLogoutTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
        ...
        [TestCase("100")]
        public void Log_Logout_TestItemsPerPageVerify(string pageSizeValue)
        {
            LogStep("Switching to Log Logout tab");
            var LogLogoutTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Logout')]")
        ...
        [TestCase("3")]
        public void Log_Logout_TestClickPageButtonIfExists(string pageNumber)
        {
            LogStep("📂 Navigating to 'Log Logout' tab");
            var LogLogoutTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Logout all')]")));
            LogLogoutTab.Click();
            WaitForUIEffect();
            ...
            {
                LogStep($"✅ Page {pageNumber} button not found — only one page exists. Skipping.");
                Assert.IsTrue(true);
                return;
            }
