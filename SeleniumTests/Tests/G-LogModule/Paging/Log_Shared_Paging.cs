...
namespace SeleniumTests.Tests.G_Log.Paging
{
    ...
    public class Log_Shared_Paging
    {
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log Shared Page - Paging"; // You can make this dynamic if needed
        ...
        public void SetUp()
        {
            ...
            _moduleName = "Log Shared Page - Paging";
        ...
        public void Log_Shared_TestPagingNextButtonAndVerify()
        {
            LogStep("📄 Switching to 'Shared' log tab");
            var LogSharedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            ...
            if (isDisabled)
            {
                _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Shared_Page_{DateTime.Now:yyyyMMdd_HHmmss}.png");
        ...
        public void Log_Shared_TestPagingPreviousButtonAndVerify()
        {
            try
            {
                LogStep("📄 Switching to 'Shared' log tab");
                ...
                try
                {
                    ...
                    {
                        LogStep("✅ Table Shared after 'Next'");
                    }
                    else
                    {
        ...
        public void Log_Shared_TestPagingClickLastIconAndVerify()
        {
            try
            {
                LogStep("Switching to Log Shared tab.");
                var LogSharedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
                By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Shared')]")));
                LogSharedTab.Click();
        ...
        public void Log_Shared_TestPagingFirstButtonAndVerify()
        {
            string tableXPath = "//table/tbody/tr[1]";

            try
            {
                LogStep("📁 Navigating to 'Shared' tab");
                var LogSharedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
                By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Shared')]")
                ));
                LogSharedTab.Click();
        ...
        [TestCase("100")]
        public void Log_Shared_TestItemsPerPageVerify(string pageSizeValue)
        {
            LogStep("📁 Navigating to 'Shared' tab");
            var LogSharedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Shared')]")
        ...
        [TestCase("3")]
        public void Log_Shared_TestClickPageButtonIfExists(string pageNumber)
        {
            LogStep("📌 Navigating to 'Shared' tab");
            var LogSharedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Shared')]")));
            ...
            LogStep("⏳ Waiting for table to update");
            bool tableShared = _wait.Until(driver =>
            {
                var SharedTable = driver.FindElement(By.XPath(tableXPath));
                string afterHtml = SharedTable.GetAttribute("innerHTML");
            ...
            var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
            File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
            Assert.IsTrue(tableShared, $"❌ Table did not update after clicking page {pageNumber}.");
        }
