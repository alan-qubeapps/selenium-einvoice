...
namespace SeleniumTests.Tests.G_Log.Paging
{
    ...
    public class Log_Deleted_Paging
    {
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log Deleted Page - Paging"; // You can make this dynamic if needed
        ...
        public void SetUp()
        {
            ...
            _moduleName = "Log Deleted Page - Paging";
        ...
        public void Log_Deleted_TestPagingNextButtonAndVerify()
        {
            LogStep("Switching to 'Deleted' tab");
            var LogDeletedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Deleted')]")));
            LogDeletedTab.Click();
            WaitForUIEffect();

            LogStep("📄 Waiting for table to be visible and capturing current HTML");
            ...
            LogStep("🔎 Locating 'Next' button and checking if enabled");
            ...
            if (isDisabled)
            {
                _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Deleted_Page_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                LogStep("⚠️ 'Next' button is disabled — only one page exists. Skipping test.");
                Assert.IsTrue(true);
                return;
            }
            ...
            bool contentChanged = _wait.Until(driver =>
            {
            ...
            Assert.IsTrue(contentChanged, "❌ Table content did not change after clicking 'Next'.");
            LogStep("✅ Table updated after pagination");
        }
        ...
        public void Log_Deleted_TestPagingPreviousButtonAndVerify()
        {
            try
            {
                LogStep("Switching to 'Deleted' tab");
                ...
                LogStep("📄 Capturing original table HTML");
                ...
                try
                {
                    ...
                    var tableAfterNext = _driver.FindElement(By.XPath(tableXPath)).GetAttribute("innerHTML");
                    ...
                    {
                        LogStep("ℹ️ Table did not change after 'Next' (may already be last page)");
                    }
                }
                catch (Exception e)
                {
                    LogStep($"⚠️ Error clicking 'Next': {e.Message}");
                }
                ...
                try
                {
                    ...
                    var tableAfterPrev = _driver.FindElement(By.XPath(tableXPath)).GetAttribute("innerHTML");
                    ...
                    {
                        LogStep("ℹ️ Table did not return to original (maybe not on page 1)");
                    }
                }
                catch (Exception e)
                {

                    LogStep($"⚠️ Error clicking 'Previous': {e.Message}");
                }
                _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Deleted_Page_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                Assert.IsTrue(true, "✅ Pagination buttons handled safely");
            }
            catch (Exception ex)
            {
                _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Deleted_Page_{DateTime.Now:yyyyMMdd_HHmmss}.png");
        ...
        public void Log_Deleted_TestPagingClickLastIconAndVerify()
        {
            try
            {
                LogStep("Switching to 'Deleted' tab");
                ...
                LogStep("📄 Capturing first row content before navigation");
                var firstRow = _wait.Until(ExpectedConditions.ElementIsVisible(By.XPath("//table/tbody/tr[1]")));
                ...
                if (isDisabled)
                {
                    ...
                    Assert.IsTrue(true, "Last button is disabled, no further paging needed.");
                }
                ...
                LogStep("⏳ Waiting for table to update after clicking 'Last'");
                bool tableChanged = false;

                try
                {
                    tableChanged = new WebDriverWait(_driver, TimeSpan.FromSeconds(15)).Until(driver =>
                    {
                ...
                if (tableChanged)
                {
                    LogStep("✅ Table content changed after clicking 'Last'");
                    Assert.IsTrue(true, "Table content successfully changed.");
                }
                else
                {
        ...
        public void Log_Deleted_TestPagingFirstButtonAndVerify()
        {
            try
            {
                LogStep("Switching to 'Deleted' tab");
                ...
                if (isLastButtonDisabled)
                {
                    ...
                    _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_OnlyOnePage_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                    var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                    File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
        ...
        [TestCase("100")]
        public void Log_Deleted_TestItemsPerPageVerify(string pageSizeValue)
        {
            LogStep("Switching to 'Deleted' tab");
            var LogDeletedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Deleted')]")));
            ...
            LogStep($"🔍 Checking if dropdown contains '{pageSizeValue}'");
            ...
            if (tableUpdated)
            {
                ...
                var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                LogStep("✅ Table updated or already within selected page size.");
                Assert.IsTrue(true);
            }
            else
            {
                ...
                var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                Assert.Fail("❌ Table did not update as expected after page size change.");
            }
        }
        ...
        [TestCase("3")]
        public void Log_Deleted_TestClickPageButtonIfExists(string pageNumber)
        {
            LogStep("Switching to 'Deleted' tab");
            var LogDeletedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
            By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Deleted')]")));
            ...
            LogStep($"🔍 Checking if page {pageNumber} button exists");
            var pageButtons = _driver.FindElements(By.XPath(dynamicPageXPath));
            if (pageButtons.Count == 0)
            {
                LogStep($"✅ Page {pageNumber} button not found — only one page available.");
                Assert.IsTrue(true);
                return;
            }
