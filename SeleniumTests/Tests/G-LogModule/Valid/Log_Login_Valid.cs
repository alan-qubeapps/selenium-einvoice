...
namespace SeleniumTests.Tests.G_Log
{
    ...
    public static class ExcelDataReaderLogLoginValid
    {
    ...
    public class Log_Login_Valid
    {
        ...
        private static string ExcelPath = Path.Combine(AppConfig.TestDataFolder, "LogLoginTestDataValid.xlsx");

        public static IEnumerable<object[]> SearchLogTestData =>
        ExcelDataReaderLogLoginValid.GetSearchLogTestData(ExcelPath, "SearchLogTestData");

        public static IEnumerable<object[]> ResetLogTestData =>
        ExcelDataReaderLogLoginValid.GetResetLogTestData(ExcelPath, "ResetLogTestData");

        public static IEnumerable<object[]> FilterUserLogTestData =>
        ExcelDataReaderLogLoginValid.GetFilterUserLogTestData(ExcelPath, "FilterUserLogTestData");
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log Login Page"; // You can make this dynamic if needed
        ...
        public void Search_Login_Log(string searchText, int columnIndex)
        {
            try
            {
                // Navigate to Login Logs Tab
                var loginTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
                By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Login')]")
                ));
                loginTab.Click();
                WaitForUIEffect();
                LogStep("✅ Switched to Login Logs tab");
        ...
        public void ExportLogLoginReport()
        {
            ...
            WaitForUIEffect();
            LogStep("📤 Clicked export button on Login Logs tab");
            ...
            try
            {
                ...
                if (modal != null)
                {
                    ...
                    _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Login_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                    var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                    File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
        ...
        public void Reset_After_Search_Login_Log(string searchText, int columnIndex)
        {
            try
            {
        ...
        public void Filter_User_Login_Log(string searchText, int columnIndex)
        {
            try
            {
                var logLoginTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
