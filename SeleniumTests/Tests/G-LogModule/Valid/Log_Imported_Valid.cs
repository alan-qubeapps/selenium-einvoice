...
namespace SeleniumTests.Tests.G_Log
{
    ...
    public static class ExcelDataReaderLogImportedValid
    {
    ...
    [AllureEpic("ERP-117")]
    public class Log_Imported_Valid
    {
        ...
        private static string ExcelPath = Path.Combine(AppConfig.TestDataFolder, "LogImportedTestDataValid.xlsx");

        public static IEnumerable<object[]> SearchLogTestData =>
        ExcelDataReaderLogAllValid.GetSearchLogTestData(ExcelPath, "SearchLogTestData");

        public static IEnumerable<object[]> ResetLogTestData =>
        ExcelDataReaderLogAllValid.GetResetLogTestData(ExcelPath, "ResetLogTestData");

        public static IEnumerable<object[]> FilterUserLogTestData =>
        ExcelDataReaderLogAllValid.GetFilterUserLogTestData(ExcelPath, "FilterUserLogTestData");
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log Imported Page"; // You can make this dynamic if needed
        ...
        public void Search_Imported_Log(string searchText, int columnIndex)
        {
            try
            {
                // Navigate to Imported Logs Tab
                var importedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
                By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Imported')]")
                ));
                importedTab.Click();
                WaitForUIEffect();
                LogStep("✅ Switched to Imported Logs tab");
		...
		[Test]
		[Category("Log Imported")]
		[Order(2)]
		[AllureSeverity(SeverityLevel.normal)]
		[AllureStory("Create - Export Log Report")]
		public void ExportLogImportedReport()
		{
			string downloadPath = AppConfig.DownloadPath;
			string filePrefix = "Log Index";
			bool isModalDisplayed = false;

			var logImportedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
			By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Imported')]")
			));

			logImportedTab.Click();
			WaitForUIEffect();
			...
			WaitForUIEffect();
			LogStep("📤 Clicked export button on Log Imported tab");

			// Check if modal appears (indicates export is processing in background)
			try
			{
				...
				if (modal != null)
				{
                    ...
                    _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Imported_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                    var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                    File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
			...
			// Proceed to download check only if no modal
			if (!isModalDisplayed)
			{
				...
				while ((DateTime.Now - startTime).TotalSeconds < 20)
				{
					var files = Directory.GetFiles(downloadPath, $"{filePrefix}*")
						.Where(f => !f.EndsWith(".crdownload")) // skip incomplete downloads
					...
					if (files.Any())
					{
						...
						if (File.GetLastWriteTime(latestFile) > startTime.AddSeconds(-5)) // must be newly downloaded
						{
							fileDownloaded = true;
							break;
						}
        ...
        public void Reset_After_Search_Imported_Log(string searchText, int columnIndex)
        {
            try
            {
                var logImportedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
                ...
                try
                {
                    tableRefreshed = new WebDriverWait(_driver, TimeSpan.FromSeconds(30)).Until(driver =>
                    {
                        var currentHtml = driver.FindElement(By.XPath(tableXPath)).GetAttribute("innerHTML");
                        return currentHtml != previousTableHtml;
                    });
        ...
        public void Filter_User_Imported_Log(string searchText, int columnIndex)
        {
            try
            {
                var logImportedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
