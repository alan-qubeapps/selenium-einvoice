...
namespace SeleniumTests.Tests.G_Log
{
    ...
    public static class ExcelDataReaderLogAllValid
    {
    ...
    public class Log_All_Valid
    {
        ...
        private static string ExcelPath = Path.Combine(AppConfig.TestDataFolder, "LogAllTestDataValid.xlsx");
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log All Page"; // You can make this dynamic if needed
        ...
        public void Search_All_Log(string searchText, int columnIndex)
        {
            try
            {
                WaitForUIEffect();
                ...
                // Wait for table auto-refresh
                bool tableRefreshed = new WebDriverWait(_driver, TimeSpan.FromSeconds(120)).Until(driver =>
                {
                ...
                if (tableRefreshed)
                    LogStep("✅ Table auto-refreshed after search.");
                ...
                {
                    Assert.Fail($"❌ No match found for '{searchText}' in column {columnIndex}.");
                }
            }
            catch (Exception ex)
            {
        ...
        [Test]
        [Category("Log Exported")]
        [Order(2)]
        [AllureSeverity(SeverityLevel.normal)]
        [AllureStory("Create - Export Log Report")]
        public void ExportLogAllReport()
        {
            ...
            try
            {
                ...
                if (modal != null)
                {
                    ...
                    okButton.Click();
                    LogStep("Export is processing in background. Skipped file check.");
                }

                // Verify file download (if modal didn’t appear)
                if (!isModalDisplayed)
                {
            ...
            catch (WebDriverTimeoutException)
            {
                ...
                var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                LogStep("✅ No modal appeared. Proceeding with file download verification.");
            }
        ...
        public void Reset_After_Search_Log(string searchText, int columnIndex)
        {
            try
            {
                WaitForUIEffect();
        ...
        public void Filter_User_Log(string searchText, int columnIndex)
        {
            try
            {
                WaitForUIEffect();
