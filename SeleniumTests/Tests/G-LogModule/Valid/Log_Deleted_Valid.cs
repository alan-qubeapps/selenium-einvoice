...
namespace SeleniumTests.Tests.G_Log
{
    ...
    public static class ExcelDataReaderLogDeletedValid
    {
    ...
    [AllureEpic("ERP-117")]
    public class Log_Deleted_Valid
    {
        ...
        private static string ExcelPath = Path.Combine(AppConfig.TestDataFolder, "LogDeletedTestDataValid.xlsx");

        public static IEnumerable<object[]> SearchLogTestData =>
        ExcelDataReaderLogDeletedValid.GetSearchLogTestData(ExcelPath, "SearchLogTestData");

        public static IEnumerable<object[]> ResetLogTestData =>
        ExcelDataReaderLogDeletedValid.GetResetLogTestData(ExcelPath, "ResetLogTestData");

        public static IEnumerable<object[]> FilterUserLogTestData =>
        ExcelDataReaderLogDeletedValid.GetFilterUserLogTestData(ExcelPath, "FilterUserLogTestData");
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log Deleted Page"; // You can make this dynamic if needed
        ...
        public void Search_Deleted_Log(string searchText, int columnIndex)
        {
            try
            {
                // Navigate to Deleted Logs Tab
                ...
                logDeletedTab.Click();
                WaitForUIEffect();
                LogStep("✅ Switched to Deleted Logs tab");
        ...
        [Test]
        [Category("Log Deleted")]
        [Order(2)]
        [AllureSeverity(SeverityLevel.normal)]
        [AllureStory("Create - Export Log Report")]
        public void ExportLogDeletedReport()
        {
            ...
            WaitForUIEffect();
            LogStep("📤 Clicked export button on Log Deleted tab");
            ...
            try
            {
                ...
                if (modal != null)
                {
                    ...
                    {
                        _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Deleted_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                        var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                        File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
        ...
        public void Reset_After_Search_Deleted_Log(string searchText, int columnIndex)
        {
            try
            {
                var logDeletedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
        ...
        public void Filter_User_Deleted_Log(string searchText, int columnIndex)
        {
            try
            {
                var logDeletedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
