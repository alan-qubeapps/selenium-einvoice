...
namespace SeleniumTests.Tests.G_Log
{
    ...
    public static class ExcelDataReaderLogUpdatedValid
    {
    ...
    [AllureEpic("ERP-117")]
    public class Log_Updated_Valid
    {
        ...
        private static string ExcelPath = Path.Combine(AppConfig.TestDataFolder, "LogUpdatedTestDataValid.xlsx");
        ...
        public void OneTimeSetUp()
        {
            ...
            string moduleName = "Log Updated Page"; // You can make this dynamic if needed
        ...
        public void Search_Updated_Log(string searchText, int columnIndex)
        {
            try
            {
                // Navigate to Updated Logs Tab
                var updatedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
                By.XPath("//a[contains(@class,'nav-link') and contains(normalize-space(text()),'Updated')]")
                ));
                updatedTab.Click();
                LogStep("✅ Switched to Updated Logs tab");
        ...
        [Test]
        [Category("Log Updated")]
        [Order(2)]
        [AllureSeverity(SeverityLevel.normal)]
        [AllureStory("Create - Export Log Report")]
        public void ExportLogUpdatedReport()
        {
            ...
            try
            {
                ...
                if (modal != null)
                {
                    ...
                    okButton.Click();
                    LogStep("Export is processing. Skipping download check.");
                }
            }
            catch (WebDriverTimeoutException)
            {
        ...
        public void Reset_After_Search_Updated_Log(string searchText, int columnIndex)
        {
            try
            {
                ...
                if (isMatchFound)
                {
                    ...
                    _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Updated_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                    var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                    File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                }
                else
        ...
        public void Filter_User_Updated_Log(string searchText, int columnIndex)
        {
            try
            {
                var logUpdatedTab = _wait.Until(ExpectedConditions.ElementToBeClickable(
