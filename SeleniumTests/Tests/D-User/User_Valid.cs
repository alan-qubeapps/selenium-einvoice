...
namespace SeleniumTests.Tests.D_User
{
    ...
    public static class ExcelDataReaderUserValid
    {
        public static IEnumerable<object[]> GetCreateUserRoleTestData(string filePath, string sheetName)
        {
            ...
            using (var package = new ExcelPackage(fileInfo))
            {
                ...
                for (int row = 2; row <= rowCount; row++)
                {
                    ...
                    {
                        Rolename, RoleDesc, colorClass, EditStorePermission
                    };
        ...
        public static IEnumerable<object[]> GetUpdateUserRoleTestData(string filePath, string sheetName)
        {
            var fileInfo = new FileInfo(filePath);
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
        ...
        public static IEnumerable<object[]> GetCreateUserTestData(string filePath, string sheetName)
        {
            var fileInfo = new FileInfo(filePath);
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
        ...
        public static IEnumerable<object[]> GetSearchUserTestData(string filePath, string sheetName)
        {
            var fileInfo = new FileInfo(filePath);
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            ...
            using (var package = new ExcelPackage(fileInfo))
            {
                ...
                for (int row = 2; row <= rowCount; row++)
                {
                    // Collect possible search texts from columns 1-3
        ...
        public static IEnumerable<object[]> GetUpdateUserTestData(string filePath, string sheetName)
        {
            var fileInfo = new FileInfo(filePath);
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            ...
            using (var package = new ExcelPackage(fileInfo))
            {
                ...
                for (int row = 2; row <= rowCount; row++)
                {
                    string User = worksheet.Cells[row, 1].Text?.Trim();
                    ...
                    yield return new object[]
                    {
                        User, Username, CustEmail, role, UserPassword, UserConfirmPassword, activeUser
                    };
        ...
        public static IEnumerable<object[]> GetSearchUserRoleTestData(string filePath, string sheetName)
        {
            var fileInfo = new FileInfo(filePath);
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            ...
            using (var package = new ExcelPackage(fileInfo))
            {
                ...
                for (int row = 2; row <= rowCount; row++)
                {
                    // Collect possible search texts from columns 1
                    var values = new List<string>
            {
                worksheet.Cells[row, 1].Text?.Trim()
            };
    ...
    public class User_Valid
    {
        ...
        private static string ExcelPath = Path.Combine(AppConfig.TestDataFolder, "UserTestDataValid.xlsx");

        public static IEnumerable<object[]> CreateUserRoleTestData =>
            ExcelDataReaderUserValid.GetCreateUserRoleTestData(ExcelPath, "CreateUserRoleTestData");

        public static IEnumerable<object[]> UpdateUserRoleTestData =>
            ExcelDataReaderUserValid.GetUpdateUserRoleTestData(ExcelPath, "UpdateUserRoleTestData");

        public static IEnumerable<object[]> CreateUserTestData =>
        ExcelDataReaderUserValid.GetCreateUserTestData(ExcelPath, "CreateUserTestData");

        public static IEnumerable<object[]> SearchUserTestData =>
            ExcelDataReaderUserValid.GetSearchUserTestData(ExcelPath, "SearchUserTestData");

        public static IEnumerable<object[]> UpdateUserTestData =>
        ExcelDataReaderUserValid.GetUpdateUserTestData(ExcelPath, "UpdateUserTestData");

        public static IEnumerable<object[]> FilterCategoryTestData =>
            ExcelDataReaderUserValid.GetFilterCategoryTestData(ExcelPath, "FilterCategoryTestData");

        public static IEnumerable<object[]> SearchUserRoleTestData =>
        ExcelDataReaderUserValid.GetSearchUserRoleTestData(ExcelPath, "SearchUserRoleTestData");
        ...
        [AllureStory("Create")]
        [TestCaseSource(nameof(CreateUserRoleTestData))]
        public void Create_User_Role(string Rolename, string RoleDesc, string colorClass, string EditStorePermission)
        {
            try
            {
                ...
                // ✅ Scroll the WHOLE WINDOW after ticking
                LogStep("📜 Scrolling entire window to bottom");
                ((IJavaScriptExecutor)_driver).ExecuteScript("window.scrollTo(0, document.body.scrollHeight);");
                Thread.Sleep(800); // Ensure UI fully rendered after scroll
                ...
                // Extra safety scroll directly to button
                ...
                LogStep("🔍 Verifying newly created user role in the table...");

                bool isMatchFound = false;


                // Search by UserRole Name
                _UserPage.SearchUserRole(Rolename);
                WaitForUIEffect(2000);
                helperFunction.WaitForUserRoleTableToLoad(_wait);
                WaitForUIEffect(2000);

                while (true)
                {
                    var rows = _driver.FindElements(By.XPath(
                        "/html/body/app-layout/div[1]/div/div/div/app-content/app-user/form/div/app-user-role-table/div/div[2]/div/div[1]/div/table/tbody/tr"));
                    ...
                    foreach (var row in rows)
                    {
                        ...
                        string actualUserRoleName = cells[0].Text.Trim();
                        string actualUserRoleDesc = cells[1].Text.Trim();


                        if (actualUserRoleName.Equals(Rolename, StringComparison.OrdinalIgnoreCase))
                        {
                            // Combined verification log
                            LogStep($"🔹 Verifying User Role '{Rolename}' -> " +
                                $"Description: Expected '{RoleDesc}', Actual '{actualUserRoleDesc}'; ");

                            if (actualUserRoleDesc.Equals(RoleDesc, StringComparison.OrdinalIgnoreCase))
                            {
                                isMatchFound = true;
                                LogStep($"🎉 All fields matched successfully for '{Rolename}'");
                            }
                            else
                            {
                                Assert.Fail($"❌ Verification failed for '{Rolename}', see log for details.");
                            }
                ...
                if (!isMatchFound)
                {
                    Assert.Fail($"❌ Customer '{Rolename}' was not found in the table after creation.");
                }

                LogStep("🎉 User Role update and verification completed successfully.");
        ...
        [AllureSeverity(SeverityLevel.normal)]
        [AllureStory("Update")]
        [TestCaseSource(nameof(UpdateUserRoleTestData))]
        public void Update_User_Role(string Rolename, string RoleDesc, string colorClass, string EditStorePermission)
        {
            try
            {
                ...
                LogStep($"Editing user role: {Rolename}");
                ...
                // ===== Step 6: Verification using input variables =====
                LogStep("🔍 Verifying newly updated user role in the table...");
        ...
        [AllureSeverity(SeverityLevel.normal)]
        [AllureStory("Create")]
        [TestCaseSource(nameof(CreateUserTestData))]
        public void Create_User(string Username, string UserEmail, string role, string UserPassword, string UserConfirmPassword, string activeUser)
        {
            try
            {
                ...
                LogStep("Enter user email");
                _UserPage.EnterCustEmail(UserEmail);
                ...
                if (!string.IsNullOrWhiteSpace(activeUser))
                {
                    ...
                    // Set status string for logging or later use
                    activeStatus = isActiveChecked ? "Active" : "Inactive";
                    LogStep($"🟢 Set active checkbox: {activeStatus}");
                }
                else
                {
                    LogStep("⚪ Active checkbox: no value, skipped");
                }
                ...
                // ===== Step 6: Verification using input variables =====
                LogStep("🔍 Verifying newly created User in the table...");

                bool isMatchFound = false;

                // Search by Username
                _UserPage.SearchUser(Username);
                WaitForUIEffect(2000);
                ...
                {
                    var rows = _driver.FindElements(By.XPath(
                        "/html/body/app-layout/div[1]/div/div/div/app-content/app-user/form/div/app-user-table/div/div/div[3]/div/div[1]/div/table/tbody/tr"));
                    ...
                    foreach (var row in rows)
                    {
                        ...
                        string actualUserName = cells[0].Text.Trim();
                        string actualUserEmail = cells[1].Text.Trim();
                        string actualUserRole = cells[2].Text.Trim();
                        string actualUserStatus = cells[3].Text.Trim();

                        if (actualUserName.Equals(Username, StringComparison.OrdinalIgnoreCase))
                        {
                            // Combined verification log
                            LogStep($"🔹 Verifying Username '{Username}' -> " +
                                $"Email: Expected '{UserEmail}', Actual '{actualUserEmail}'; " +
                                $"Role: Expected '{role}', Actual '{actualUserRole}'; " +
                                    $"Status: Expected '{activeStatus}', Actual '{actualUserStatus}'; ");

                            if (actualUserEmail.Equals(UserEmail, StringComparison.OrdinalIgnoreCase)
                                && actualUserRole.Equals(role, StringComparison.OrdinalIgnoreCase)
                                && actualUserStatus.Equals(activeStatus, StringComparison.OrdinalIgnoreCase))
                            {
                                isMatchFound = true;
                                LogStep($"🎉 All fields matched successfully for '{Username}'");
                            }
                            else
                            {
                                Assert.Fail($"❌ Verification failed for '{Username}', see log for details.");
                            }
                ...
                if (!isMatchFound)
                {
                    Assert.Fail($"❌ User '{Username}' was not found in the table after creation.");
                }

                LogStep("🎉 User created and verification completed successfully.");
        ...
        [TestCaseSource(nameof(SearchUserTestData))]
        public void Search_User(string searchText)
        {
        ...
        [TestCaseSource(nameof(UpdateUserTestData))]
        public void UpdateUser(string User, string Username, string UserEmail, string role, string UserPassword, string UserConfirmPassword, string activeUser)
        {
            try
            {
                var wait = new WebDriverWait(_driver, TimeSpan.FromSeconds(15));

                LogStep($"Click [Edit] for user: {User}");
                ...
                // ===== Step 6: Verification using input variables =====
                LogStep("🔍 Verifying newly updated User in the table...");
                ...
                LogStep("🎉 User updated and verification completed successfully.");
        ...
        [AllureSeverity(SeverityLevel.normal)]
        [AllureStory("User Search - General Match (Partial Match Accepted)")]
        [TestCaseSource(nameof(SearchUserRoleTestData))]
        public void Search_User_Role(string searchText)
        {
            var wait = new WebDriverWait(_driver, TimeSpan.FromSeconds(10));
            ...
            LogStep($"🔍 Searching for user with text: {searchText}");
            _UserPage.SearchUserRole(searchText);
            helperFunction.WaitForUserRoleTableToLoad(_wait);
            WaitForUIEffect();
